...
Использование Grafan'ы позволит наблюдать за занимаемым местом на диске, использованием CPU, временем отклика, количеством ответов с различными статусами (в особенности нас интересующими - 5xx). К тому же, Granafa имеет функцию отправления alert при нарушения установленных нами допустимых диапазонов каких-либо метрик. В общем, настроит непрерывное обозрение за состоянием системы.
...
Для повышения надёжности в сохранении данных при падении какой-либо из БД будет использоваться технология репликации.
 - Mongo DB replicaset
     Для механизма репликации в MongoDB будет использоваться ReplicaSet, который позволяет быстро выбрать Primary Node из Secondary при падении текущего Primary. К тому же, в нем есть подход HeartBeat, при котором Secondary Nodes слушают друг друга для отлавливания случая падений узлов. Быстрота выбора Primary Node обеспечивается засчет наличия легковесного Arbiter, который проводит "голосование" за выбор главного узла. 
 - Redis HA (High Availability)
     Данная конфигурация обеспечивает мало того, что быструю репликацию между Main и Secondary nodes, но к тому же и надёжную, при которой исключена возможность потери данных при реплицирование. Вдобавок, в данной конфигурации присутствует функции обнаружении и устранения сбоев.
...
## Golang profiling + tracing (Jaeger)

Для определения возможных утечек памяти, неоптимизированных участков кода, неэффективного обращения к памяти и тд следует использовать профилирование в Golang. К тому же, для определения особо времезатратных сервисов/функций/участков кода лучше использовать Jaeger, который подробно анализирует время выполнения запроса.
...
[1. Паттерн Circuit Breaker](https://habr.com/ru/companies/otus/articles/778574/)

[2. Fine Tuning](https://habr.com/ru/companies/okmeter/articles/423085/)

[3. Kafka Reliability](https://under-sky-web.ru/post/kafka-kak_obespechit_nadejnost_i_neprotivorechivost_soobscheniy)

[4. Профилирование и оптимизация web-приложений на Go](https://habr.com/ru/companies/badoo/articles/324682/)

[5. MongoDB ReplicaSet Deployment](https://www.geeksforgeeks.org/replica-set-deployment-in-mongodb/) 

[6. Введение в distributed tracing](https://habr.com/ru/companies/oleg-bunin/articles/649203/)

[7. Jaeger for tracing in microservice architecture](https://habr.com/ru/companies/yandex_praktikum/articles/729906/)
