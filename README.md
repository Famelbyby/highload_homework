...
Для балансировки внешних запросов будет использоваться такой L7 load balancer, как Nginx. Он был выбран по следующим причинам:
  - Nginx умеет архивировать ответы с помощью Gzip
...
## Внутренние запросы
Для балансировки внутренних запросов на уровне L4 будет использоваться LVS via Direct Routing, ведь он высокопроизводителен, так как изменяет лишь MAC-адрес, не затрагивая IP. Минусом является то, что тогда все серверы нужно будет сделать в одной физической сети. К тому же, на уровне L7 будет затронут Nginx, использующий метод наименьших соединений (Least connections).
## Схема отказоустойчивости
Отказоустойчивость будет обеспечена засчет нескольких пунктов:
  1. Использование Kubernetes - он позволяет автоматически перезапускать серверы при их падении
  ...
  3. К тому же, периодически может проводиться проверка работоспособности сервера посредством отсылания на него базовых GET-запросов
