...
![image](https://github.com/user-attachments/assets/48559a25-076a-4731-a46f-bf3fb26cc2b1)
...
|:-----------------------:|:--------------------------:|:----------------------------------:|
| Session                 | session_id - уникальный хеш сессии<br>user_id - пользователь данной сессии | В таблице хранятся сессии и соответствующие им пользователи |
...
|:-----------------------:|:-----------------------------------:|
| Session                 | Допустим, длина хеша - 40 символов. Будем считать, что сессия длится день. Тогда размер данных: 34 млн (DAU) * (5 + 8) байт = 0.41 Гб |
...
|:-----------------------:|:-------------------------------------:|:----------------------------:|
| Session                 | Сессия проверяется при каждом запросе, поэтому RPS - 8 730. Тогда нагрузка на чтение составляет: 8 730 * (5 + 8) байтов = 0.1 Мб/сек | 34 млн (DAU) в день - то есть 393 RPS * (5 + 8) байт = 4.99 Кб/сек |
...
| **Итого**               | 110.4 Мб/сек | 244 Кб/сек |

## Указать требования к консистентности
Рассмотрим условия консистентности внтури таблиц.

Таблица 15 - Условия консистентности внутри таблиц
| Имя таблицы             | Условия консистентности               |
|:-----------------------:|:-------------------------------------:|
| Session                 | Сессия уникальна<br>Пользователь уникален |
| User                    | У каждого пользователя уникальный ID<br>У каждого пользователя уникальный хеш на аватарку<br>У каждого пользователя уникальный никнейм |
| Follow                  | Пользователь не может быть подписан на себя<br>Пара (followed_user_id, following_user_id) должны быть уникальны |
| Pin                     | У каждого пина уникальный ID<br>У каждого файла уникальный хеш<br>Дата изменения не раньше даты создания |
...
Таблица 16 - Условия консистентности между таблицами
| Имя таблицы             | Размер данных таблицы               |
|:-----------------------:|:-----------------------------------:|
...
Таблица 17 - Условие консистентности таблицы по времени
...
Можно выделить следующие особенности:
  1. Относительно большая нагрузка на чтение и маленькая нагрузка на запись в таблице Pins_tags. Ключ для чтения: tag_id bigint.
  2. Аналогичная ситуация в таблице Saved_pins. Ключ для чтения: user_id bigint.
