...
*Примечание:* 
  - хранилищами для файлов является S3 хранилище
...
Шардирование таблиц будет сделано по следующим критериям:
  ...
  - таблицу пинов шардировать по updated_at
...
# 7. Алгоритмы
### Просмотр ленты
Подбор пинов при загрузке ленты будет организован на подходе Content-Based filtering. То есть, обращаясь к полю preferred_tags, мы получаем "вектор интересов" пользователя, на основе которого (допустим, по методу k-ближайших соседей) находим подходящие пины. Поиск пинов реализуется через полнотекстовый поиск тегов в поле tags
### Добавление пина в сохранённые
Прилетает запрос на добавление пина в сохранённые. Пока пин добавляется в таблицу saved_pins с данными pin_id и user_id, параллельно происходит пересчёт "вектора интересов" пользователя по пинам с полученными от пина тегами.
### Хранение пинов
